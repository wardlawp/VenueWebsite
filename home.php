...
<html>
	...
	<body>
			...
			if(isset($venues)){
				if(!empty($venues)){
					...
					foreach ($venues as $venue)
					{
						...
						echo '<a class="VenueLink" href="venue.php"> ' .$number. " .{$venue['name']} . '</a>";
						echo '<p class ="description"><em>Address:</em> ' .$venue['address'] . '</p>';
						echo '<p class ="description"><em>Category:</em> ' . rtrim($venue['categories'], ',') . '</p>';
						echo '<a class="mapLink" value="'.$venue['lat'] . ',' . $venue['long'] .  ',' . $venue['name'] .  ',' . $number.  '"><button class="button"style="float:right;" type="button">Show</button> </a>';
						echo '<a   target="_blank" href="' ."http://maps.google.com/maps?z=12&t=m&q=loc:{$venue['lat']}+{$venue['long']}" . '"><button class="button" style="background-color:#00cc44; float:right;" type="button">Directions</button> </a>';
						echo '</div>';
						$number += 1;
					}
				} else {
		...
		<div class="BottomBar" >
			...
			<p>Home | <a href="about.php">About Us</a></p>
			<p> 
